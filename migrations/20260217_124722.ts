...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE INDEX \`posts_rss_image_idx\` ON \`posts\` (\`rss_image_id\`);`)
  ...
  await db.run(sql`CREATE INDEX \`_posts_v_version_version_rss_image_idx\` ON \`_posts_v\` (\`version_rss_image_id\`);`)
  ...
  `)
  await db.run(sql`CREATE INDEX \`site_settings_intro_lines_order_idx\` ON \`site_settings_intro_lines\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`site_settings_intro_lines_parent_id_idx\` ON \`site_settings_intro_lines\` (\`_parent_id\`);`)
  ...
  `)
  await db.run(sql`CREATE INDEX \`site_settings_profile_avatar_idx\` ON \`site_settings\` (\`profile_avatar_id\`);`)
}
