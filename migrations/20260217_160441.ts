...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `)
  await db.run(
    sql`INSERT INTO \`__new_comments\`("id", "scope", "post_id", "author_name", "author_email", "content", "status", "reply_content", "reply_replied_at", "reply_replied_by_id", "updated_at", "created_at") SELECT "id", 'post', "post_id", "author_name", "author_email", "content", "status", "reply_content", "reply_replied_at", "reply_replied_by_id", "updated_at", "created_at" FROM \`comments\`;`
  )
...
export async function down({
  ...
  `)
  await db.run(
    sql`INSERT INTO \`__new_comments\`("id", "post_id", "author_name", "author_email", "content", "status", "reply_content", "reply_replied_at", "reply_replied_by_id", "updated_at", "created_at") SELECT "id", "post_id", "author_name", "author_email", "content", "status", "reply_content", "reply_replied_at", "reply_replied_by_id", "updated_at", "created_at" FROM \`comments\`;`
  )
  await db.run(sql`DROP TABLE \`comments\`;`)
  await db.run(sql`ALTER TABLE \`__new_comments\` RENAME TO \`comments\`;`)
  await db.run(sql`PRAGMA foreign_keys=ON;`)
