...
import BlogListLink from './blog-list-link'
...
export default function BlogListClient({
  ...
  return (
    <section className={'mt-3'}>
      {firstBlog && (
        <BlogListLink slug={firstBlog.slug}>
          <div className="flex w-full flex-col">
            {firstBlog.metadata.image && (
              ...
              >
                <img
                  ...
                  width={
                    placeholderImageBlogMap.get(firstBlog.slug)?.metadata
                      ?.width || 1920
                  }
                  height={
                    placeholderImageBlogMap.get(firstBlog.slug)?.metadata
                ...
                <Image
                  className="pointer-events-none absolute top-[-85px] right-4 z-10 w-20"
            ...
            <div className="flex items-center gap-1">
              <p className="text-neutral-600 text-xs dark:text-neutral-400">
                {blogPostDate(
                  firstBlog.metadata.publishedAt,
                  dateLocale,
                  timeZone
                )}
              </p>
      ...
      <section className={clsx('mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2')}>
        {restBlogs.map((post) => {
          return (
            <BlogListLink key={post.slug} slug={post.slug}>
              <div className="flex w-full flex-col">
                {post.metadata.image && (
                  <div
                    className={
                  ...
                  >
                    ...
                    <img
                      ...
                      width={
                        placeholderImageBlogMap.get(post.slug)?.metadata
                          ?.width || 1920
                      }
                      height={
                        placeholderImageBlogMap.get(post.slug)?.metadata
